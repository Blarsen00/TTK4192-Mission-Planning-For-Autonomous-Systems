...
class gridWorld(object):
    ...
    def actionToIndex(self, a = None):
        """
        Returns the index correcposning to the action provided. 
        """
        assert(a in ["U", "D", "L", "R"]), f"{a} is not an action"
        return ["U", "D", "L", "R"].index(a)
        
    def indexToAction(self, idx):
        lst = ["U", "D", "L", "R"]
        return lst[idx]
    
    def validateAction(self, a, s=None):
        """
        Returns a boolean to check if the action a is valid in position s. If 
        s is None, then s is set to current position. A action is valid if the 
        next state following the action is within the grid, and if the next position
        is not an obstacle.
        """
        ...
        pos = tuple([sum(x) for x in zip(s, move[a])])
        if pos in self.legal_states:
            return True
