...
"""
# ------------------------- Global parameters ------------------------------
Easily modify the parameters of the pathfind by tuning the following parameters.
The waypoints revert back to the original for the Gazebo simulation. And for the 
physical model. Scale is for the path finding plot found in task 6b.
"""
# Global scale parameter that scales the entire environment
SCALE = 20
ORIGINAL_MAP_DIMENSION=(5.0, 2.9)
MAP_DIMENSION=tuple(x*SCALE for x in ORIGINAL_MAP_DIMENSION)
CAR_LEN = 0.5
# Tighter turns make for more trajectories which slows down execution time
MAX_PHI = pi/5

# Paramaters for changing the resolution of the path search algorithm
ORIGINAL_CELL_SIZE=0.25
CELL_SIZE=0.1*SCALE
DT=1e-2
# STEP_SIZE defines how many waypoints we move for given step.
STEP_SIZE=1
...
WAYPOINTS = [[0.2, 0.2, 0], [1.85, 0.6-0.1, pi/2], [3, 0.91+0.1, 3*pi/2], 
             [3.25, 2.6, pi/2], [4.8, 0.4, 0], [0.87, 2.4+0.1, pi], [3.65, 1.75-0.1, pi/2], 
             [0.8, 0.5, pi/4], [2.38, 0.43, pi/2], [2.42, 1.22, 0], 
             [3.02, 1.35, pi/2], [3.9, 1.0, pi]]

WAYPOINTS_MOVE = []

# We store the original, unscaled waypoints for robot execution as those are unscaled
ORIGINAL_WAYPOINTS = WAYPOINTS.copy()

# Important to scale the waypoints according to our global scaling variable
WAYPOINTS = np.array(WAYPOINTS)
WAYPOINTS[:,:2] = WAYPOINTS[:,:2] * SCALE
...
def read_plan():
    plan_general = []
    filename = '~/catkin_ws/src/AI-planning/tmp_sas_plan.1'
...
def main_hybrid_a(heu,start_pos, end_pos,reverse, extra, grid_on, 
    ...
    tc = map_grid_robplan(scale=scale)

    # Check to see if we want to use the scaled version, or original
    if scale:
        env = Environment_robplan(tc.obs, lx=MAP_DIMENSION[0], ly=MAP_DIMENSION[1])
        grid = Grid_robplan(env, cell_size=CELL_SIZE)
    else:
        env = Environment_robplan(tc.obs,   lx=ORIGINAL_MAP_DIMENSION[0], 
                                            ly=ORIGINAL_MAP_DIMENSION[1])
        grid = Grid_robplan(env, cell_size=ORIGINAL_CELL_SIZE)

    car = SimpleCar(env, start_pos, end_pos, l=CAR_LEN, max_phi=MAX_PHI)
    ...
    xl_np=xl_np-env.lx/2
    yl_np=np.array(yl_np1)
    yl_np=yl_np-env.ly/2
    ...
    # Plotting the waypoints given in the task
    for wp in WAYPOINTS[:7]:
        ax.plot(wp[0], wp[1], 'ro', markersize=6)
    # Plotting the waypoints for improved search
    for wp in WAYPOINTS[7:]:
    ...
    if task6b:
        ax.plot(xl[:len(path)], yl[:len(path)], color=np.array([51, 255, 51])/255)
...
class map_grid_robplan:
    def __init__(self, scale=False):
        ...
        obs.extend([
            [ORIGINAL_WAYPOINTS[6][0]-0.35, 1.75, 0.8, 0.5],
            [ORIGINAL_WAYPOINTS[1][0]-0.45, 0.6, 0.9, 0.3],
            [ORIGINAL_WAYPOINTS[1][0]-0.45+0.9+0.2, 0.6, 0.9, 0.3],
        ])
...
class turtlebot_move():
    ...
    def __init__(self):
        ...
        self.y = 0.0
        self.theta = 0.0
        self.pid_theta = PID(PID_P,PID_I,PID_D)  # initialization
        ...
        for point in WAYPOINTS_MOVE[::STEP_SIZE]:
    ...
    def move_to_point(self, x, y):
        ...
        self.pid_theta.setPID(PID_P_STEERING, PID_I_STEERING, PID_D_STEERING)
        # P control while steering
...
# -------------- Example from arm_control.py ------------------------
    ...
    # rospy.sleep(2)
# -------------- Example from arm_control.py ------------------------
    print("Moving the robot arm to inspect a valve....")
    time.sleep(5)
...
def move_robot(start_pos, end_pos, experiment=False, scale=False, task6b=False):
    """
    Generates a path from start to end and moves along that path. 
    The scale parameter tells us if we have a scaled environment or not.
    """
    if experiment:
...
def map_direction(command : str):
    """
    Function to map the desired angle for each of the waypoints. 
    Manually implemented in order to help the search algorithm as it has been
    struggling with finding a path when there is obstacles in the way. 

    One should note that the commands need better names. Moving from wp7 to wp5
    should be named something like m75, as opposed to r21, but it is a bit too
    late to bother changing it. It would have made this function alot easier
    to read though.

    The function takes the command, which is a string that describes the movement
    from one waypoint to another. The mapping is found in the PDDL problem, but 
    the comments should be clear as to what what is happening. 

    We only change the orientation of the destination, not the startposition, as that
    would compute a path that does not correspond to the current position of the robot.
    """
...
def mission_execution(plan_general, scale=False, task6b=False):
    print("")
    ...
    while i_ini < task_total:
        ...
        if plan_temp[0]=="picture":
            ...
            move_robot(start_pos=start_pos, end_pos=end_pos, scale=scale, task6b=task6b)
            time.sleep(1)
            ...
            print(f"Could not find an action: {plan_temp} :: {plan_temp[0]}")
            raise
...
# Old version without the computed task with intermediary points
# def task6b():
#     # Waypoint 1 to 2, 2 to 3, 3 to 4
#     points = WAYPOINTS[0:4]
#     for i, point in enumerate(points):
#         if i == (len(points)-1):
#             return
#         start_pos = point
#         end_pos = points[i+1]
#         move_robot(start_pos=start_pos, end_pos=end_pos, scale=True, task6b=True)


def task6b():
    """
        Computed the general plan that corresponds to task 6b and then we execute said plan
    """
    plan_general = ["move r03 wp1 wp8", "move r26 wp8 wp9", "move r09 wp9 wp2",
            "move r07 wp2 wp10", "move r15 wp10 wp3", 
            "move r13 wp3 wp10", "move r24 wp10 wp6", "move r23 wp6 wp11", 
            "move r19 wp11 wp4"]
    mission_execution(plan_general, scale=True, task6b=True)
...
if __name__ == '__main__':
    try:
        ...
        print(f"Map dimensions: {MAP_DIMENSION}")
        print(f"Car length: {CAR_LEN}")
        print(f"Parameters for search algorithm:")
        print(f"Cell size: {CELL_SIZE}")
        print(f"dt: {DT}")
        print()
        print("**************************************************************")
        ...
        # 5.01) Running through waypoints 1->4 for task 6b
        print("-------------------- Performin task 6b --------------------")
        task6b()
        print("-------------------- Finished task 6b --------------------")
	...
	# 5.1) Starting the AI Planner
       #Here you must run your AI planner module
        ...
        mission_execution(plan_general)
