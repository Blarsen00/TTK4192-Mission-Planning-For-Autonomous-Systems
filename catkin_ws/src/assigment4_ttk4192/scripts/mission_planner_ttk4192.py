...
#----------------------------  IMPORTED Hybrid A* algorithm below ---------------------------- 
    ...
    def search_path(self, heu=1, extra=False):
        ...
        return None, None
#----------------------------  IMPORTED Hybrid A* algorithm above ---------------------------- 

def main_hybrid_a(heu,start_pos, end_pos,reverse, extra, grid_on):

    tc = map_grid_robplan()
    # ---------- I can't find these _robplan structures anywhere ----------      
...
class map_grid_robplan:
    def __init__(self):
        ...
        """ 
        ---------- Waypoints given in the assignment. All orientations are set to pi ----------      
        Add some value +-0.1 to each of the waypoints. Use common sense so you don't cross 
        the map limits.
        """
        # w0 = [0.2, 0.2, pi]
        # w1 = [1.85, 0.5, pi]
        ...
        self.start_pos2 = [0.2, 0.2, pi]
        self.end_pos2 = [3.65, 1.75, pi]
        ...
        # Obstacles defined by wayoints
        self.obs.extend([
            [WAYPOINTS[6][0]-0.4, WAYPOINTS[6][1], 0.8, 0.5], 
            [WAYPOINTS[1][0]-0.45, WAYPOINTS[1][1]+0.15, 0.9, 0.5], 
            [WAYPOINTS[2][0]-0.45+0.9+0.2, WAYPOINTS[2][1]-0.15, 0.9, 0.3], 
        ]
        )
...
def move_robot_waypoint0_waypoint1():
    ...
    # start_pos = [2, 2, 0]
    # end_pos = [6, 6, 3*pi/4]
    start_pos = [2*0.1, 2*0.1, 0.0]
    end_pos = [6*0.1, 2*0.1, 0.0]
    # end_pos = [6*0.1, 6*0.1, 3*pi/4]
